...
(function (global, doc, ibexa, Routing, Translator) {
    ...
    const previewForm = doc.querySelector('.ibexa-perso-scenario-preview__form');
    let searchInput = null;
    const fetchData = () => {
        const isFormValid = previewForm.reportValidity();
    ...
    const fetchAttributes = (event) => {
        ...
        const { outputTypeId } = option.dataset;
        let requestInput;
    ...
    const addTag = ({ label, value, container }) => {
        ...
        if (tagTemplate) {
            ...
            renderedTemplate = inputTemplate
                .replace('__name__', tagIndex)
                .replace('__name__', label)
                .replace('__name__', label)
                .replaceAll('__name__', tagIndex);
        }
    ...
    const checkIsAlreadySelected = (item) => {
        const container = contextItemsSearchInput.closest('.ibexa-perso-scenario-preview__context-items');
        const isAlreadySelectedItem = container.querySelector(`${SELECTOR_TAG_INPUT}[value="${item.id.replace(/["\\]/g, '\\$&')}"]`);

        return isAlreadySelectedItem;
    };
    const prepareItemsForShow = (items) => {
        return items.filter((item) => !checkIsAlreadySelected(item));
    };
    const onConfirmContextItems = (item) => {
        const isAlreadySelectedItem = checkIsAlreadySelected(item);
    ...
    const copyResponseCodeToClipboard = () => {
        if (!global.navigator.clipboard) {
            ibexa.helpers.notification.showErrorNotification(
                Translator.trans(
                    /*@Desc("Cannot copy the response due to browser limitations. Select and copy manually.")*/ 'scenarios.preview.copy_clipboard_error',
                    {},
    ...
    previewForm.addEventListener('submit', (event) => event.preventDefault(), false);
