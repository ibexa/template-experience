...
(function(doc, ibexa, Routing, Translator, React, ReactDOM) {
    ...
    const onConfirmContexItems = (button, items) => {
        ...
        closeUDW();
        items.forEach((item) => {
            const contentName = ibexa.helpers.text.escapeHTML(item.ContentInfo.Content.Name);
    ...
    const buildCategoryPathTagLabel = (viewData) => {
        ...
        return searchHitList
            .map((searchHit) => ibexa.helpers.text.escapeHTML(searchHit.value.Location.ContentInfo.Content.TranslatedName))
            .join(' / ');
    };
...
})(window.document, window.ibexa, window.Routing, window.Translator, window.React, window.ReactDOM);
