(function(doc, Routing, eZ, Translator, React, ReactDOM) {
    ...
    const openUdwButton = doc.querySelector('.ibexa-btn--add-items-udw');
    const NOMINAL_TYPE = 'NOMINAL';
    const CLASS_EMPTY_TAG = 'ez-submodel__item--empty';
    const SELECTOR_SAVE_BUTTON = '.ibexa-btn--save';
    const SELECTOR_SAVE_AND_CLOSE_BUTTON = '.ibexa-btn--save-close';
    ...
    const SELECTOR_REMOVE_GROUP_ITEM_BUTTON = '.ibexa-tag__remove-btn';
    ...
    const fetchSubmodelAvailableItems = () => {
        const { customerId } = doc.querySelector('.ez-personalization-base-item__form').dataset;
    ...
    const setSubmodelData = () => {
        ...
        const { type, attributeSource, source } = option.dataset;
