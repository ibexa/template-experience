(function(doc, eZ, bootstrap) {
    ...
    const actionCheckboxes = doc.querySelectorAll('.ez-field-edit__data-group-items--action-checkbox-container .ibexa-input--checkbox');
    const openModalButtons = doc.querySelectorAll('.ez-strategy-models__open-modal-button');
    const discardSettingsButtons = doc.querySelectorAll('.ibexa-modal__discard-settings-button');
    ...
    const onSubmitClick = (event) => {
        ...
        if (isFormValid) {
            ...
            doc.querySelectorAll('.ibexa-tabs__tab').forEach((navItem) => {
                navItem.classList.remove('ibexa-tabs__tab--invalid');
            });

            invalidTabs.forEach((invalidInputs) => {
                invalidInputs.forEach((invalidInputKey) => {
                    doc.querySelector(`#item-${invalidInputKey}`).classList.add('ibexa-tabs__nav-item--invalid');
                });
            });
        }
    };
...
})(window.document, window.eZ, window.bootstrap);
