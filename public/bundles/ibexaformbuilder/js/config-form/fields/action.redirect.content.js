(function (global, doc, Translator, ibexa) {
    ...
    const openUdw = (event) => {
        ...
        const title = Translator.trans(/*@Desc("Select content")*/ 'action.redirect.udw.title', {}, 'ibexa_form_builder');
    ...
    const setSelectedItem = (items) => {
        ...
        const itemName = ibexa.helpers.text.escapeHTML(selectedItem.ContentInfo.Content.TranslatedName);
        const itemType = ibexa.helpers.text.escapeHTML(selectedItem.ContentInfo.Content.ContentTypeInfo.names.value[0]['#text']);
