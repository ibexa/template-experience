    ...
    template: `
     <div class="editprice">
        ...
        <div class="form-group mt-4 mb-4 row" v-if="allowsearch == 'true'">
          <div class="col-sm-3">
            <label class="ibexa-filters__item-label">{{ translate('label.sku') }} *</label>
              <input type="text" name="sku.query" v-model="priceSearch.sku" id="" placeholder="type sku" v-on:keyup.enter="searchSku(priceSearch.sku)" class="ibexa-filters__input form-control">
          </div>
          <div class="col-sm-3">
            <button class="btn ibexa-btn ibexa-btn--primary mt-4"   @click="searchSku(priceSearch.sku)">
        ...
        <p v-if="allowsearch == 'true'">
         ...
         <div v-if="message.type != ''">
          ...
          <div class="ibexa-table ibexa-table-subitems" v-if="priceSearch.status == 1 && priceSearch.variants.length == 0" >

          <div class="ibexa-table-header" v-if="priceSearch.status == 1">
            ...
            <div>
                <button type="submit" href="#" @click="addPriceLine(emptyLine,'-')" class="btn ibexa-btn ibexa-btn--primary ibexa-btn--no-text">
        ...
        <div class="ibexa-table ibexa-table-subitems" v-if="priceSearch.status == 1 && priceSearch.variants.length != 0">
          ...
          <div class="tab-pane fade show active" v-bind:id="'tab-'+variantCode"  v-for="(variant,variantCode) in priceSearch.variants">
              ...
              <div class="ibexa-table-header" >
                ...
                <div>
                    <button type="submit" href="#" @click="addPriceLine(emptyLine,variantCode)" class="btn ibexa-btn ibexa-btn--primary ibexa-btn--no-text">
                        <svg class="ibexa-icon ibexa-icon--small ibexa-icon--create">
                            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="/bundles/ibexaadminui/img/ibexa-icons.svg#create"></use>
                        </svg>
                    </button>
        ...
        <div v-if="show_extra_add_button">
          <span>
            <button type="submit" href="#" @click="addPriceLine({sku: priceSearch.result.sku, variantCode: '', shopId: null})" class="btn ibexa-btn ibexa-btn--primary">
        ...
        <div v-if="priceSearch.status == 1">
          <span>
            <button type="submit" href="#" @click="updatePrices()" class="btn ibexa-btn ibexa-btn--primary">
