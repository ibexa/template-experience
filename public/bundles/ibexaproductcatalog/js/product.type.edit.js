(function(global, doc, eZ) {
    ...
    const HIGHLIGHT_GROUP_TIME = 1000;
    const REMOVE_PLACEHOLDERS_TIME = 500;
    ...
    const shouldHideAddedGroup = (attributesGroupNode) => {
        const addedAttributes = attributesGroupNode.querySelectorAll('.ibexa-pc-assigned-attribute:not(.ibexa-pc-assigned-attribute--in-removing)');

        return !addedAttributes.length;
    };
    const shouldHideEmptyDropZoneMessage = () => {
        const addedAttributes = doc.querySelectorAll('.ibexa-pc-attributes-drop-zone .ibexa-pc-assigned-attribute:not(.ibexa-pc-assigned-attribute--in-removing)');
    ...
    const findAttributeAfterPosition = (attributesGroupNode, targetAttributePosition) => {
        ...
        return (
            [...addedAttributes].find((attributeNode) => {
    ...
    class AttributesDraggable extends eZ.core.Draggable {
        onDrop() {
            const { type, attributeId, attributeGroupId, attributePosition, groupId } = currentlyDraggedItem.dataset;

            if (type === ATTRIBUTE_DRAG_TYPE) {
                const targetAttributesGroup = dropZoneContainer.querySelector(`[data-group-id="${attributeGroupId}"]`);
                const sourceAttributeGroup = doc.querySelector(
                    `.ibexa-available-attribute-item-group[data-group-id="${attributeGroupId}"]`,
                );
                ...
                const targetInsertContainer = targetAttributesGroup.querySelector(
                    '.ibexa-pc-added-attributes-group__list .ibexa-table__body',
                );

                targetInsertContainer.insertBefore(
                    attributeToAssign,
                    findAttributeAfterPosition(targetAttributesGroup, attributePosition),
                );
        ...
        onDragOver(event) {
            ...
            emptyDropZoneMessageNode.classList.add('ibexa-pc-attributes-empty-drop-zone--hidden');
        ...
        removePlaceholder() {
            ...
            dropZoneContainer.classList.remove('ibexa-pc-attributes-drop-zone--active');
            attributesGroups.forEach((attributeGroup) => attributeGroup.classList.remove('ibexa-pc-added-attributes-group--active'));
        }
    }
    ...
    const dropAttributesWidget = new AttributesDraggable({
        ...
        timeoutRemovePlaceholders: REMOVE_PLACEHOLDERS_TIME,
    });
