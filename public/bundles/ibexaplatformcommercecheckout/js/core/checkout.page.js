    ...
    class CheckoutPage {
        ...
        refreshBasketData() {
            this.getBasket().then((response) => {
                const basketContainer = doc.querySelector('.ezcommerce-basket-summary');
                const shippingLines = basketContainer.querySelectorAll('.ezcommerce-basket-summary__row--shipping');
                const { template } = basketContainer.dataset;
                const fragment = doc.createDocumentFragment();
                const totalLineValue = basketContainer.querySelector('.ezcommerce-basket-summary__row-value--total');
