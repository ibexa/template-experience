    ...
    class CheckoutPage {
        ...
        refreshBasketData() {
            this.getBasket().then((response) => {
                ...
                const vatLines = basketContainer.querySelectorAll('.ibexa-commerce-basket-summary__row--vat');
                ...
                vatLines.forEach((vatLine) => vatLine.remove());
                response.basketData.vatLines.forEach((vatLine) => {
                    const container = doc.createElement('div');
                    const renderedItem = template
                        .replace('{{ NAME }}', vatLine.vatPercentage)
                        .replace('{{ PRICE }}', vatLine.vatValue);

                    container.insertAdjacentHTML('beforeend', renderedItem);
                    container.querySelector('.ibexa-commerce-basket-summary__row').classList.add('ibexa-commerce-basket-summary__row--vat');
                    fragment.append(container.querySelector('div'));
                });
