(function (global, doc, ibexa, Translator) {
    ...
    const openUdw = (itemsContainer, openUdwBtn, draggable, event) => {
        ...
        const title = Translator.trans(/*@Desc("Select content")*/ 'config_form.widgets.collection.udw.title', {}, 'ibexa_page_builder');
    ...
    const addItems = (itemsContainer, openUdwBtn, draggable, items) => {
        ...
        items.forEach((item) => {
            ...
            const itemLabelNameNode = listItem.querySelector('.ibexa-pb-collection-item__label-name');
    ...
    const removeItem = (itemsContainer, openUdwBtn, event) => {
        ...
        const item = event.currentTarget.closest(SELECTOR_ITEM);

        item.dispatchEvent(new CustomEvent('ibexa-drag-and-drop:start-removing'));
        item.dispatchEvent(
            new CustomEvent('ibexa-drag-and-drop:end-removing', {
                detail: {
                    callback: () => {
                        updateInputValue(itemsContainer, openUdwBtn);
                    },
