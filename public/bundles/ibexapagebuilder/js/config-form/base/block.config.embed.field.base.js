(function (global, doc, ibexa, Translator) {
    ...
    ibexa.BlockConfigEmbedFieldBase = class BlockConfigEmbedFieldBase {
        ...
        updateFieldValue(btn, items) {
            ...
            const location = this.getLocation(items);
            ...
            const embeddedItemActionsContainer = metaContainer.querySelector('.ibexa-embedded-item-actions');
            const embedMetaActionsNode = metaContainer.querySelector('.ibexa-pb-embed-meta__actions');
            const { embeddedItemActionsTemplate } = metaContainer.dataset;

            embeddedItemActionsContainer.remove();
            embedMetaActionsNode.insertAdjacentHTML('beforeend', embeddedItemActionsTemplate);

            const itemActionsMenuContainer = metaContainer.querySelector('.ibexa-embedded-item-actions .ibexa-multilevel-popup-menu');
            const itemActionsTriggerElement = metaContainer.querySelector(
                '.ibexa-embedded-item-actions .ibexa-embedded-item-actions__menu-trigger-btn',
            );
            const languageCodes = contentInfo.CurrentVersion.Version.VersionInfo.VersionTranslationInfo.Language.map(
