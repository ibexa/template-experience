...
(function (global, doc, ibexa) {
    const SELECTOR_REVEAL = '.ibexa-pb-reveal-hide-date-time__reveal';
    const SELECTOR_HIDE = '.ibexa-pb-reveal-hide-date-time__hide';
    const SELECTOR_PICKER = '.ibexa-picker';
    const SELECTOR_PICKER_FORM_INPUT = '.ibexa-picker__form-input';
    ...
    const getNowAccurateToMinutes = () => {
        ...
        const userTimezone = getAdminUiConfig().timezone;
        const dateWithUserTimezone = convertDateToTimezone(now, userTimezone);
    ...
    const getPickerFlatpickrInstance = (pickerComponent) => {
        const pickerWidget = ibexa.helpers.objectInstances.getInstance(pickerComponent);
