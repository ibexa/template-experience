...
(function (global, doc, React, ReactDOM, ibexa) {
    ...
    const saveValueToForm = () => {
        const items = selectedEntries.map((entry) => {
            ...
            if (entry.internalItem) {
                entryName = entry.internalItem.name;
            } else {
                const selectedTagNode = doc.querySelector(`.ibexa-tag-view-select__selected-item-tag[data-id="${entry.id}"]`);
                const selectedTagName = selectedTagNode.dataset.name;

                entryName = selectedTagName;
            }
