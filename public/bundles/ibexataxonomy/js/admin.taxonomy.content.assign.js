(function (global, doc, ibexa, React, ReactDOM, Translator) {
    ...
    const openUDW = (event) => {
        const defaultUniversalDiscoveryTitle = Translator.trans(
            /*@Desc("Select Location")*/ 'add_location.title',
            {},
            'universal_discovery_widget',
        );
        const { udwConfig, universaldiscoveryTitle } = event.currentTarget.dataset;
        const config = JSON.parse(udwConfig);
        const title = universaldiscoveryTitle ?? defaultUniversalDiscoveryTitle;
