...
const SelectParentTree = (props) => {
    ...
    const moduleName = Translator.trans(/*@Desc("Taxonomy tree")*/ 'taxonomy.tree_name', {}, 'ibexa_taxonomy_tree');

    useEffect(() => {
        loadTreeToState().then((rootItem) => {
            window.document.dispatchEvent(
                new CustomEvent(`ibexa-tree-builder:${moduleId}:set-expanded-nodes`, {
                    detail: { items: [rootItem] },
                }),
            );
        });
    }, []);
