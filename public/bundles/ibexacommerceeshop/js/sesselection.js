(function(global, doc) {
    const CLASS_HIDDEN = 'ibexa-data-source__options--hidden';
    const CLASS_SELECTED = 'option-selected';
    const SELECTOR_FIELD = '.ibexa-field-edit--sesselection';
    const SELECTOR_OPTIONS = '.ibexa-data-source__options';
    const SELECTOR_SELECTED = '.ibexa-data-source__selected';
    const SELECTOR_SOURCE_INPUT = '.ibexa-data-source__input';
    const EVENT_VALUE_CHANGED = 'change';

    class EzSelectionValidator extends global.ibexa.BaseFieldValidator {
        /**
        ...
        validateInput(event) {
            ...
            const errorMessage = global.ibexa.errors.emptyField.replace('{fieldName}', label);
    ...
    [...doc.querySelectorAll(SELECTOR_FIELD)].forEach((container) => {
        const createSelectedItem = (value, label) =>
            `<li class="selected-item" data-value="${value}">${label}<span class="remove-selection"></span></li>`;
        ...
        const clearCurrentSelection = () => {
            ...
            [...container.querySelectorAll(`${SELECTOR_OPTIONS} .option-selected`)].forEach((option) =>
        ...
        const handleClickOutside = (event) => {
            ...
            hideOptions();
            container.querySelector('.ibexa-data-source__input').dispatchEvent(new CustomEvent(EVENT_VALUE_CHANGED));
        };
        ...
        [...container.querySelectorAll(`${SELECTOR_OPTIONS} li`)].forEach((option) =>
            option.addEventListener('click', handleClickOnOption, false)
        );
    });
})(window, document);
