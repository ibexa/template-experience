(function(global, doc, eZ) {
    ...
    const deleteLanguage = (event) => {
        ...
        widget.querySelector('.ibexa-btn--add-language').removeAttribute('disabled');
    ...
    const saveNewLanguage = (event) => {
        ...
        const languageButtons = modal.querySelectorAll('.ibexa-btn--save-language');
        const listContainer = widget.querySelector('.ez-site-factory-form-languages__items');
        ...
        const deleteButton = htmlWidget.querySelector('.ibexa-btn--delete-language');
        ...
        if (items.length === languageButtons.length) {
            widget.querySelector('.ibexa-btn--add-language').setAttribute('disabled', true);
        }
    };
    const hideModal = (modal) => {
        modal.classList.remove('ez-site-factory-language-selector__list-wrapper--visible');
        modal.querySelectorAll('.ibexa-btn--save-language').forEach((btn) => btn.removeEventListener('click', saveNewLanguage));
    ...
    const showAddLanguageModal = (event) => {
        ...
        languageButtons.forEach((btn) => {
            if (languages.includes(btn.dataset.languageCode)) {
                btn.classList.add('ibexa-btn--save-language--hidden');
            } else {
                btn.classList.remove('ibexa-btn--save-language--hidden');
    ...
    const attachEventToAddButtons = () => {
        const addButtons = doc.querySelectorAll('.ibexa-btn--add-language');

        addButtons.forEach((btn) => btn.addEventListener('click', showAddLanguageModal));
    };
    const attachEventToDeleteButtons = () => {
        const deleteButtons = doc.querySelectorAll('.ibexa-btn--delete-language');
    ...
    doc.body.addEventListener('ez-site-factory-add-public-access', (event) => {
        event.detail.widget.querySelector('.ibexa-btn--add-language').addEventListener('click', showAddLanguageModal);
    });
