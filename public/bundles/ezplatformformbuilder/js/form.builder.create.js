(function(global, doc, eZ, React, ReactDOM, Translator) {
    const CLASS_FORM_BUILDER_CLOSED = 'ibexa-form-builder--closed';
    const CLASS_FORM_BUILDER_VISIBLE = 'ibexa-form-builder-modal--visible';
    const CLASS_SCROLL_DISABLED = 'ez-scroll-disabled';
    const CLASS_FIELD_NO_VALUE = 'ibexa-form-builder--no-value';
    const SELECTOR_MODAL = '.ibexa-form-builder-modal';
    ...
    const fieldTypeContainer = doc.querySelector('.ibexa-form-builder');
    const formBuilderContainer = doc.querySelector('#ibexa-form-builder-root');
    ...
    const closeFormBuilder = () => {
        ...
        const modal = fieldTypeContainer.querySelector('.ibexa-form-builder-modal');
    ...
    const attachEventHandlers = (btn) => {
        ...
        const btnRemove = fieldTypeContainer.querySelector('.ibexa-form-builder-preview__action--trash');
    ...
    const updatePreview = () => {
        const formFieldsConfig = formFieldsConfigInput.value ? JSON.parse(formFieldsConfigInput.value) : null;
        const formHasAnyFields = formFieldsConfig && formFieldsConfig.fields && formFieldsConfig.fields.length > 0;
        const callToAction = doc.querySelector('.ibexa-form-builder__call-to-action');
