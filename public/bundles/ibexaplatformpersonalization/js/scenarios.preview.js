...
(function(doc, getComputedStyle, eZ, Routing, Translator, React, ReactDOM) {
    ...
    const { customerId } = doc.querySelector('.ez-scenario-preview__container--recomendation-response').dataset;
    ...
    const fetchData = () => {
        ...
        if (isFormValid) {
            ...
            const request = new Request(Routing.generate('ibexa.personalization.recommendation.preview', { customerId: customerId, name: scenarioName }), {
                method: 'POST',
                headers: {
    ...
    const addTag = ({ label, value, container }) => {
        ...
        const lastTagAfterInsert = tagsContainer.querySelector(`${SELECTOR_TAG}:last-of-type`);
...
})(window.document, window.getComputedStyle, window.eZ, window.Routing, window.Translator, window.React, window.ReactDOM);
